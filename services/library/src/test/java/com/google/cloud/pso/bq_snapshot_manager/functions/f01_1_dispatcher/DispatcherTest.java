...
public class DispatcherTest {
    ...
    @Test
    public void testDispatcherWithTableIncludeList() throws IOException, NonRetryableApplicationException, InterruptedException {
                ...
                return new PubSubPublishResults(
                        Arrays.asList(
                                new SuccessPubSubMessage(
                                        new DispatcherTableRequest("test-run-id",
                                                false,
                                                false,
                                                null,
        ...
        // there should be only one published message to Dispatcher Table that contains the tableIncludeList
        assertEquals(results.getSuccessMessages().size(), 1);
        DispatcherTableRequest dispatcherTableRequest = ((DispatcherTableRequest) results
                .getSuccessMessages()
                .get(0)
                .getMsg());
    ...
    @Test
    public void testDispatcherWithDatasetIncludeList() throws IOException, NonRetryableApplicationException, InterruptedException {

    BigQueryScope bigQueryScope =
        new BigQueryScope(
            new ArrayList<>(), // folders - should have no effect
            Arrays.asList("p1", "p2"), // projects include - should be the entry point effect
            new ArrayList<>(), // projects exclude
        ...
        List<String> actual = results
                .getSuccessMessages()
                 .stream()
                 .map(x -> ((DispatcherTableRequest) x.getMsg()).getDatasetSpec().toSqlString())
