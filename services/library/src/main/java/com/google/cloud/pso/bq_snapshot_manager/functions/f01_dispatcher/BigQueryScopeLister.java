...
public class BigQueryScopeLister {
   ...
   *
   * <p>Detecting which resources to list is done bottom up TABLES > DATASETS > PROJECTS > FOLDERS
   * where lower levels configs (e.g. Tables) ignore higher level configs (e.g. Datasets) For
   * example: If TABLES_INCLUDE list is provided: * List only these tables * SKIP tables in
   * TABLES_EXCLUDE list * IGNORE all other INCLUDE lists If DATASETS_INCLUDE list is provided: *
   * List only tables in these datasets * SKIP datasets in DATASETS_EXCLUDE * SKIP tables in
   * TABLES_EXCLUDE * IGNORE all other INCLUDE lists If PROJECTS_INCLUDE list is provided: * List
   * only datasets and tables in these projects * SKIP datasets in DATASETS_EXCLUDE * SKIP tables in
   * TABLES_EXCLUDE * IGNORE all other INCLUDE lists If FOLDERS_INCLUDE list is provided: * List
   * only projects, datasets and tables in these folders * SKIP projects in PROJECTS_EXCLUDE * SKIP
   * datasets in DATASETS_EXCLUDE * SKIP tables in TABLES_EXCLUDE * IGNORE all other INCLUDE lists
   *
   * @param bqScope
   * @return List<TableSpec>
   * @throws NonRetryableApplicationException
   */
  public List<TableSpec> listTablesInScope(BigQueryScope bqScope)
      throws NonRetryableApplicationException {
    ...
    List<Pattern> tableExcludeListPatterns =
        extractAndCompilePatterns(bqScope.getTableExcludeList(), REGEX_PREFIX);
    List<Pattern> datasetExcludeListPatterns =
        extractAndCompilePatterns(bqScope.getDatasetExcludeList(), REGEX_PREFIX);
    List<Pattern> projectExcludeListPatterns =
        extractAndCompilePatterns(bqScope.getProjectExcludeList(), REGEX_PREFIX);
  ...
  private List<TableSpec> processTables(
    ...
    for (String table : tableIncludeList) {
      ...
      try {
        Tuple<Boolean, String> checkResults =
            isIncluded(table, tableExcludeList, tableExcludeListPatterns);
        if (!checkResults.x()) {
  ...
  private List<TableSpec> processDatasets(
    ...
    for (String dataset : datasetIncludeList) {
      ...
      try {
        ...
        Tuple<Boolean, String> checkResults =
            isIncluded(dataset, datasetExcludeList, datasetExcludeListPatterns);
        ...
        if (!checkResults.x()) {
          ...
          if (datasetTables.isEmpty()) {
            String msg = String.format("No Tables found under dataset '%s'", dataset);
            ...
            logger.logInfoWithTracker(
                runId,
                null,
                String.format("Found %s tables under dataset %s", datasetTables.size(), dataset));
          }
        } else {
          logger.logInfoWithTracker(
              runId,
    ...
    return processTables(tablesIncludeList, tableExcludeList, tableExcludeListPatterns);
  }
  ...
  private List<TableSpec> processProjects(
    ...
    for (String project : projectIncludeList) {
      try {
        ...
        Tuple<Boolean, String> checkResults =
            isIncluded(project, projectExcludeList, projectExcludeListPatterns);
  ...
  private Tuple<Boolean, String> isIncluded(
